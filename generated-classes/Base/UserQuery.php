...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'leno', $modelName = '\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT user_id, email, name, age, password FROM user WHERE user_id = :p0';
        try {
    ...
    public function filterByTask($task, $comparison = null)
    {
        if ($task instanceof \Task) {
            return $this
                ->addUsingAlias(UserTableMap::COL_USER_ID, $task->getCreatorId(), $comparison)
                ->addUsingAlias(UserTableMap::COL_USER_ID, $task->getHelperId(), $comparison);
        } else {
