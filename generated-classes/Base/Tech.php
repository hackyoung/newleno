...
abstract class Tech implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\TechTableMap';
    ...
    /**
     * Initializes internal state of Base\Tech object.
     */
     ...
     * @return $this|\Tech The current object (for fluent API support)
     */
    public function setTechId($v)
    {
        ...
        if ($this->tech_id !== $v) {
            $this->tech_id = $v;
            $this->modifiedColumns[TechTableMap::COL_TECH_ID] = true;
        }
    ...
    public function setDeleted($v)
    {
        ...
        if ($this->deleted !== null || $dt !== null) {
            if ($this->deleted === null || $dt === null || $dt->format("Y-m-d H:i:s") !== $this->deleted->format("Y-m-d H:i:s")) {
                ...
                $this->modifiedColumns[TechTableMap::COL_DELETED] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : TechTableMap::translateFieldName('TechId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->tech_id = (null !== $col) ? (int) $col : null;
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : TechTableMap::translateFieldName('Deleted', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Tech'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(TechTableMap::COL_DELETED)) {
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'tech_id':
                        $stmt->bindValue($identifier, $this->tech_id, PDO::PARAM_INT);
                        break;
                    case 'label':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TechTableMap::COL_TECH_ID)) {
            $criteria->add(TechTableMap::COL_TECH_ID, $this->tech_id);
        }
        if ($this->isColumnModified(TechTableMap::COL_LABEL)) {
        ...
        if ($this->isColumnModified(TechTableMap::COL_DELETED)) {
            $criteria->add(TechTableMap::COL_DELETED, $this->deleted);
        }
    ...
    /**
     * Generic method to set the primary key (tech_id column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Tech (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setTechId(NULL); // this is a auto-increment column, so set to default value
        }
    }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Tech Clone of current object.
     * @throws PropelException
     */
