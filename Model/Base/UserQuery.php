 ...
 * @method     ChildUserQuery orderByRemoved($order = Criteria::ASC) Order by the removed column
 *
 ...
 * @method     ChildUserQuery groupByRemoved() Group by the removed column
 *
 ...
 *
 * @method     ChildUserQuery leftJoinTask($relationAlias = null) Adds a LEFT JOIN clause to the query using the Task relation
 * @method     ChildUserQuery rightJoinTask($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Task relation
 * @method     ChildUserQuery innerJoinTask($relationAlias = null) Adds a INNER JOIN clause to the query using the Task relation
 *
 * @method     ChildUserQuery joinWithTask($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Task relation
 *
 * @method     ChildUserQuery leftJoinWithTask() Adds a LEFT JOIN clause and with to the query using the Task relation
 ...
 * @method     ChildUser findOneByRemoved(string $removed) Return the first ChildUser filtered by the removed column *
 ...
 * @method     ChildUser requireOneByRemoved(string $removed) Return the first ChildUser filtered by the removed column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUser[]|ObjectCollection findByRemoved(string $removed) Return ChildUser objects filtered by the removed column
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'leno', $modelName = '\\Model\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, email, name, age, password, created, updated, removed FROM user WHERE id = :p0';
        try {
    ...
    public function filterByRemoved($removed = null, $comparison = null)
    {
        if (is_array($removed)) {
            ...
            if (isset($removed['min'])) {
                $this->addUsingAlias(UserTableMap::COL_REMOVED, $removed['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($removed['max'])) {
                $this->addUsingAlias(UserTableMap::COL_REMOVED, $removed['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::COL_REMOVED, $removed, $comparison);
    }
    ...
    public function filterByTask($task, $comparison = null)
    {
        if ($task instanceof \Model\Task) {
            return $this
                ->addUsingAlias(UserTableMap::COL_ID, $task->getCreatorId(), $comparison);
    ...
    public function filterByOrder($order, $comparison = null)
    {
        if ($order instanceof \Model\Order) {
            return $this
                ->addUsingAlias(UserTableMap::COL_ID, $order->getBossId(), $comparison)
                ->addUsingAlias(UserTableMap::COL_ID, $order->getWorkerId(), $comparison);
        } else {
            throw new PropelException('filterByOrder() only accepts arguments of type \Model\Order');
        }
    }
